...
class PrivilegedTaskRunnerHelper: NSObject, RemoteProcessProtocol, NSXPCListenerDelegate {
    ...
    func runCommand(path: String, authData: NSData?, reply: @escaping (String) -> Void) {
        ...
        if authData?.length == 0 || authData?.length ?? 0 != kAuthorizationExternalFormLength {
        ...
        let authExt: UnsafeMutablePointer<AuthorizationExternalForm> = UnsafeMutablePointer.allocate(capacity: Int(kAuthorizationExternalFormLength) * MemoryLayout<AuthorizationExternalForm>.size)
